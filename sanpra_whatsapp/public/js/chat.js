...
cur_frm.cscript.custom_whatsapp_chat = function(doc) {
    show_whatsapp_chat_dialog(cur_frm);
};
        ...
        fields: [
            {
                ...
                label: 'Phone Number',
                reqd: 1,
                default: frm.doc.contact_number || frm.doc.mobile || frm.doc.contact_phone || frm.doc.phone || ''
            },
            {
...
function send_whatsapp_attachment(frm, phone_number, caption, file, dialog) {
    // Create FormData to handle file upload and attach it to the WhatsApp Chat doc
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.message && response.message.file_url) {
                // Now send the WhatsApp message with the file URL
                frappe.call({
                    method: 'send_doc_message',
                    doc: frm.doc,
                    args: {
...
function init_whatsapp_chat(frm, wrapper) {
    ...
    wrapper.find('.send-btn').on('click', function() {
        ...
        const phone = frm.doc.contact_number || frm.doc.mobile || frm.doc.contact_phone || frm.doc.phone || '';
